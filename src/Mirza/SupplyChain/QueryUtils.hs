...
import           Mirza.SupplyChain.AppConfig    (AppError, DB, Env, pg)
...
findInstLabelId :: InstanceLabelEPC -> DB Env AppError (Maybe SB.PrimaryKeyType)
...
findInstLabelId' :: GS1CompanyPrefix
                ...
                -> Maybe ItemReference
                -> Maybe AssetType
                -> DB Env AppError (Maybe SB.PrimaryKeyType)
...
getUser :: M.EmailAddress -> DB Env AppError (Maybe M.User)
...
findClassLabelId :: ClassLabelEPC -> DB Env AppError (Maybe SB.PrimaryKeyType)
...
findLabelId :: LabelEPC -> DB Env AppError (Maybe SB.PrimaryKeyType)
findLabelId (IL l)   = findInstLabelId l
findLabelId (CL c _) = findClassLabelId c

getParentId :: DWhat -> DB Env AppError (Maybe SB.PrimaryKeyType)
...
insertDWhat :: Maybe SB.PrimaryKeyType
            -> DWhat
            -> SB.PrimaryKeyType
            -> DB Env AppError SB.PrimaryKeyType
...
insertDWhen :: DWhen -> SB.PrimaryKeyType -> DB Env AppError SB.PrimaryKeyType
...
insertDWhy :: DWhy -> SB.PrimaryKeyType -> DB Env AppError SB.PrimaryKeyType
...
insertDWhere :: DWhere -> SB.PrimaryKeyType -> DB Env AppError ()
...
findDWhere :: SB.PrimaryKeyType -> DB Env AppError (Maybe DWhere)
...
findDWhereByLocationField :: MU.LocationField -> SB.PrimaryKeyType -> DB Env AppError [SB.WhereT Identity]
...
insertEvent :: SB.PrimaryKeyType -> T.Text -> Event -> DB Env AppError SB.PrimaryKeyType
...
selectUser :: M.UserID -> DB Env AppError (Maybe SB.User)
...
getEventList :: SB.LabelId -> DB Env AppError [Ev.Event]
...
findEvent :: SB.EventId -> DB Env AppError (Maybe Ev.Event)
...
isExistingContact :: M.UserID -> M.UserID -> DB Env AppError Bool
