...
import           Mirza.SupplyChain.AppConfig              (AppErr (..), DB,
                                                           SCSContext, asks, pg,
                                                           scryptPs)
...
insertUser :: Scrypt.EncryptedPass -> M.NewUser -> DB SCSContext AppErr M.UserID
insertUser encPass (M.NewUser phone (M.EmailAddress email) firstName lastName biz _) = do
  ...
  where
    errHandler (AppErr (DatabaseError e)) = throwAppErr $ case constraintViolation e of
...
newUser :: M.NewUser -> DB SCSContext AppErr M.UserID
...
-- system easily?
authCheck :: M.EmailAddress -> M.Password -> DB SCSContext AppErr (Maybe M.User)
authCheck e@(M.EmailAddress email) (M.Password password) = do
  ...
  case r of
    [user] ->
        ...
        of
          (False, _     ) -> throwAppErr $ AuthFailed (M.EmailAddress email)
    ...
    [] -> throwAppErr $ EmailNotFound e
    _  -> throwBackendError r -- multiple elements

addPublicKey :: M.User -> RSAPubKey -> DB SCSContext AppErr M.KeyID
addPublicKey (M.User (M.UserID uid) _ _)  rsaPubKey = do
  ...
  case r of
    ...
    _       -> throwAppErr . InvalidKeyID . M.KeyID $ keyId

getPublicKey :: M.KeyID -> DB SCSContext AppErr M.PEM_RSAPubKey
...
getPublicKeyInfo :: M.KeyID -> DB SCSContext AppErr M.KeyInfo
...
getEventJSON :: EvId.EventID -> DB SCSContext AppErr T.Text
getEventJSON eventID = do
  ...
  case r of
    ...
    _           -> throwAppErr $ InvalidEventID eventID

insertObjectEvent :: M.User
                  -> M.ObjectEvent
                  -> DB SCSContext AppErr Ev.Event
insertObjectEvent
...
listEvents :: LabelEPC -> DB SCSContext AppErr [Ev.Event]
listEvents labelEpc =
...
addContact :: M.User -> M.UserID -> DB SCSContext AppErr Bool
...
removeContact :: M.User -> M.UserID -> DB SCSContext AppErr Bool
...
listContacts :: M.User -> DB SCSContext  AppErr [M.User]
...
-- TODO: Write tests
listBusinesses :: DB SCSContext AppErr [SB.Business]
listBusinesses = do
...
eventUserSignedList :: EvId.EventID -> DB SCSContext AppErr [(M.User, Bool)]
...
eventsByUser :: M.UserID -> DB SCSContext AppErr [Ev.Event]
